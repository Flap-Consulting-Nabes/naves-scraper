...
def _check_for_ban(url: str, html: str, title: str) -> None:
    ...
    # F5/Incapsula (reese84) — "Pardon Our Interruption"
    if "pardon our interruption" in title_lower or "pardon our interruption" in html_lower:
        raise ScrapeBanException(f"F5/Incapsula detectado en {url} — re-ejecuta save_session.py y navega a naves-industriales manualmente")
    # Kasada — respuesta 429 con x-kpsdk-ct
    ...
    if "geetest" in html_lower and ("captcha" in html_lower or "captcha-box" in html_lower):
        raise ScrapeBanException(f"GeeTest captcha en {url} — re-ejecuta save_session.py")
    # Sesión expirada
...
@retry(
    ...
    retry=retry_if_not_exception_type(
        (ScrapeBanException, SessionExpiredException, ListingNotFoundException)
    ),
    reraise=True,
)
